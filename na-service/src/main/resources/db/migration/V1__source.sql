...
create table content_tag (id bigint not null auto_increment primary key, type varchar(20) not null, `value` varchar(255) unique not null);
...
create table news_note (id bigint not null auto_increment primary key, title varchar(255) not null, url varchar(255) not null unique, description text null, description text varchar(255) null, creation_date datetime(6), update_date datetime(6), id_source_page bigint not null);
...
alter table category_block add foreign key (id_source_page) references source_page(id);
alter table category_block_tag add foreign key (id_block) references content_block(id);
alter table category_block_tag add foreign key (id_tag) references content_tag(id);
